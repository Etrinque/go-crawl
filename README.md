...
Go-Crawl is a command-line web crawler that recursively discovers and maps internal links on a website. It uses goroutines and channels to crawl multiple pages concurrently while maintaining thread safety through proper synchronization primitives.
...
- **Concurrency Patterns**: Worker pool pattern with semaphore-like channel for controlling concurrent HTTP requests
...
- **Worker Pool Size**: Too many workers may overwhelm the target server or exhaust local resources. Recommended: 5-20 workers
...
- No persistence - state is lost when program exits
