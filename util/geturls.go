...
func GetUrlsFromHTML(htmlBody string, baseUrl *url.URL) ([]string, error) {
	...
	if err != nil {
		...
		//go_crawl.errLog = append(go_crawl.errLog, fmt.Errorf("failure to parse html body. error: %w", err))
		return nil, err
	}
		...
		if n.Type == html.ElementNode && n.Data == "a" {
			for _, a := range n.Attr {
				if a.Key == "href" {
					...
					if err != nil {
						// Log error and skip
						//go_crawl.errLog = append(go_crawl.errLog, fmt.Errorf("error parsing href: %w", err))
						continue
					}
